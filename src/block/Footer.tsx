...
const Footer = async () => {
  ...
  const footer = messages.Footer as FooterContent
    ...
    <footer className="bg-op-base relative w-full overflow-hidden">
      <div className="mx-auto max-w-[1320px] px-7 pt-16">
        <div className="flex flex-wrap items-end justify-between gap-8 pb-10">
          <p className="text-op-muted max-w-[34ch] text-[1.0625rem]">{footer.blurb}</p>
          <div className="flex flex-col items-start gap-[14px]">
            <a
              href={localize(footer.cta.href)}
            ...
            <div className="text-op-dim2 flex gap-5 font-mono text-[0.875rem]">
              <a href={config.social.emailHref} className="hover:text-op-cyan transition-colors">
                Email
              </a>
              <a
        ...
        >
          <span style={{ fontSize: 'clamp(3rem,18vw,16rem)' }}>{footer.wordmark}</span>
        </GlitchText>
      </div>
    </footer>
  )
